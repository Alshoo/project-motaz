  ...
  return (
    <div className="container w-full m-auto px-4 md:px-4 relative">
      {loading ? (
        ...
        <div>
          <h1 className="my-10 text-primary font-bold text-base sm:text-lg md:text-xl">{questDet.questionText}</h1>
        </div>
      ) : (
        <>
          {mode === "question" && (
          ...
          <h5 className="text-end text-base sm:text-lg mb-4 md:mb-14 mt-3 md:mt-5 me-2 md:me-5">
          ...
          {mode === "question" && (
            <div className="mt-5 ml-2 md:mt-14 md:ml-4 text-xs sm:text-sm md:text-base shadow-lg rounded-xl p-4 bg-white">
            ...
            <div className="mt-5 md:mt-14 space-y-2 md:space-y-4 text-xs sm:text-sm md:text-base shadow-lg rounded-2xl bg-white">
              {questDet.answers.map((ans, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`relative ${selectedAnswer === ans.id ? answerColor : ""}`}
                  >
                    ...
                    {detailsVisible[ans.id] && (
                      <div
                        className={`mt-2 p-4 shadow-inner text-left text-xs sm:text-sm md:text-base prose ${
          ...
          <div className="my-8 flex flex-col md:flex-row justify-evenly items-center gap-2 md:gap-4">
            {questDet.current_page > 1 ? (
              <button
                ...
                className="h-8 md:h-[50px] bg-primary text-white px-2 md:px-7 py-0 md:py-4 rounded-xl flex justify-center items-center text-xs sm:text-sm md:text-base"
              >
                Back
              </button>
            ) : (
              <button
                disabled
                className="h-8 md:h-[50px] bg-primary text-white px-2 md:px-7 py-0 md:py-4 rounded-xl flex justify-center items-center disabled:opacity-50 text-xs sm:text-sm md:text-base"
              >
                Back
              </button>
            )}
            <div className="rounded-full md:text-lg px-2 md:px-10 py-3 text-center relative border shadow-md">
              <button
              ...
              >
                <p className="flex items-center text-xs sm:text-sm md:text-lg text-stone-500 w-fit py-1 md:py-3 rounded-full">
      ...
      {showOverallExplanation && (
        <div className="mt-2 border border-gray-200 bg-white p-4 rounded-lg shadow-inner text-xs sm:text-sm md:text-base prose">
