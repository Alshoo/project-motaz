...
function CardStore({ title, pricing_plans, subject_ID, questions_count, chapters, Subject_Image }) {
  ...
  const isDisabled = subSubject === subject_ID;

  return (
    <div className={`grid grid-cols-1 md:grid-cols-[1fr_2fr_1fr] gap-4 bg-white rounded shadow-lg border border-gray-300 p-4 ${isDisabled ? "opacity-50" : ""}`}>
      <div className="flex justify-center items-center">
        <img className="object-cover rounded-lg w-40 md:w-48 h-28 md:h-32"
      ...
      <div className="flex flex-col justify-center items-start">
        <div className="mb-4 gap-2 grid text-xs md:text-sm font-bold text-gray-900 justify-center items-center">
          ...
          <button onClick={togglePopup} disabled={isDisabled} className={`mt-4 w-full lg:w-[75%] font-light px-3 py-2 border border-transparent bg-primary text-white rounded-lg shadow-md transition duration-300 ${isDisabled ? "" : "hover:border-primary hover:text-primary hover:bg-transparent"}`}>
            {isDisabled ? "Subscribed" : "Explore Packages"}
          </button>
        </div>
      </div>
      <div className="flex flex-col md:items-end items-center justify-center relative mb-5 md:mb-0 ">
        <p className="text-black opacity-40 text-sm md:text-base font-semibold absolute top-0">Start at {pricing_plans[0]?.total_price} LYD</p>
      </div>
      {isPopupOpen && (
