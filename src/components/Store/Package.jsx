...
function Package({ closePopup, pricing_plans, title, subject_ID, questions_count, chapters }) {
  ...
  const handlePayment = async () => {
    try {
      ...
      // Update the current plan so that the purchased plan becomes disabled
    ...
    <>
      ...
      <div className='py-5 px-5 lg:w-[35%] w-80 h-[80%] shadow-2xl bg-white overflow-y-auto rounded-lg'>
        ...
        <div className='gap-5 grid'>
          <h2 className='text-primary text-2xl font-bold'>Please Choose The Package</h2>
          <div className='gap-2 grid'>
            {pricing_plans.length > 0 &&
              pricing_plans.map((item, index) => {
                const isCurrentPlan = item.id === currentPlanId;
                return (
                  <div key={index} className={`flex items-center px-4 py-2 rounded-lg ${isCurrentPlan ? "opacity-20 border border-gray-400" : "bg-gray-200 border border-gray-400"}`}>
          ...
          <div className="w-full text-center lg:text-start bg-gray-200 rounded-lg my-4 p-5">
            <p className="flex justify-center text-primary text-lg font-bold">Coupon</p>
            <div className='mt-4'>
              <input
                type="text"
                placeholder="Enter Coupon Code"
                className="w-full p-3 rounded-lg bg-gray-100 border-gray-200 focus:border-primary placeholder-opacity-75"
                value={couponCode}
          ...
          {selectedPlan && (
            <div className="w-full text-center lg:text-start border border-gray-300 bg-transparent rounded-lg my-4 p-5">
        ...
        <div className='flex justify-center items-center'>
          <button onClick={handlePayment} type="button" disabled={insufficientBalance}
            className="mt-4 text-white bg-primary font-bold rounded-lg text-lg px-[4rem] py-1 mb-2 transition duration-300 hover:bg-transparent border hover:border-primary hover:text-black disabled:opacity-50">
            Pay
          </button>
        </div>
      </div>
