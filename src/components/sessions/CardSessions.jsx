...
function CardSessions({ statusText, buttonText, mode, question_count, created_at, updated_at, chapters, subject, Session_id, examsItems }) {
  ...
  const filteredExams = exams.filter((exam) => examsItems.includes(exam.id));
    ...
    <div className="w-[100%] grid grid-cols-1 md:grid-cols-[1fr_2fr_1fr] gap-4 bg-white rounded shadow-lg border border-gray-300 p-4">
      ...
      <div className="flex flex-col justify-center items-start">
        <div className="mb-2 flex flex-wrap gap-2 text-xs md:text-sm font-bold text-gray-900 justify-center items-center">
          <h3 className={`py-1 px-2 rounded text-white ${statusColor} text-xs md:text-sm`}>{statusText}</h3>
          <h3 className="text-black font-bold text-xs md:text-sm">{mode == 1 ? `study Mode with ${question_count} questions` : `Exam Mode with ${question_count} questions`}</h3>
        </div>
        <ul className="text-gray-700 grid gap-1 text-xs md:text-sm">
          ...
          <li className="truncate">
            <span className="text-primary font-bold">Topics </span>: {filteredExams.length > 0 ? filteredExams.map((item) => item.name + " ") : "Loading..."}
          </li>
          <li>
